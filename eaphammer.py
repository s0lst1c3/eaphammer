    ...
    try:
        ...
        if options['auth'] == 'open':
            ...
            conf_manager.hostapd_wpe_cnf.configure(interface=options['interface'],
                                            ...
                                            channel=options['channel'],
                                            wpa=options['wpa'])
        utils.Hostapd.hardstart('-s %s' % config.hostapd_cnf)
        ...
        utils.Dnsmasq.hardstart('-C %s' % config.dnsmasq_cnf)
        utils.Dnsspoof.hardstart('-i %s -f %s' % (options['interface'], config.dnsspoof_cnf))
...
def reap_creds():
    conf_manager.hostapd_wpe_cnf.configure(interface=options['interface'],
                                    ...
                                    channel=options['channel'],
                                    wpa=options['wpa'])
    utils.Hostapd.hardstart(config.hostapd_cnf)
